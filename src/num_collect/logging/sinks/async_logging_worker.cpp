...
/*!
 * \file
 * \brief Definition of functions to control workers to perform asynchronous
 * logging.
 */
...
void init_async_logging_worker(const async_logging_worker_config& config) {
    (void)async_logging_worker::instance(config);
}

void stop_async_logging_worker() { async_logging_worker::instance().stop(); }
