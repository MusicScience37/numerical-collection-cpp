...
class IwyuProcessError(RuntimeError):
    """Errors in IWYU process."""


def remove_correct_lines(target: str) -> str:
    """Remove lines that indicate correct includes.

    Args:
        target (str): Original string.

    Returns:
        str: String with correct lines removed.
    """
    result = ""
...
async def apply_iwyu_to_file(
    ...
    """Apply IWYU to a single file.

    Args:
        filepath (str): Path to the target file.
        build_dir (str): Path to the build directory.
        tqdm_obj (tqdm.tqdm): tqdm object for progress bar.
        limiter (asyncio.Semaphore): Semaphore to limit concurrent jobs.
        stop_on_error (bool): Whether to stop on the first error.
    """
...
def filter_iwyu_process_error(exc):
    """Filter out IwyuProcessError from exceptions.
...
async def apply_iwyu_to_files(
    ...
    """Apply IWYU to multiple files concurrently.
...
def get_files_in(path: pathlib.Path) -> list[str]:
    ...
    Args:
        path (pathlib.Path): Path to the directory.
...
def check_iwyu(file_or_directory_paths: list[str], build_dir: str, num_jobs: int):
    ...
    global IS_SUCCESS  # pylint: disable=global-statement,global-variable-not-assigned
