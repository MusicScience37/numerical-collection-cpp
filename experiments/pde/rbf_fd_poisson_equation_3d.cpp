...
/*!
 * \file
 * \brief Experiments to solve the Poisson equation in 3D with RBF-FD method.
 */
#include <cmath>
#include <string>
#include <tuple>
...
static auto test_function_laplacian(const variable_type& variable) -> double {
    constexpr double coeff =
        -3.0 * num_collect::base::pi<double> * num_collect::base::pi<double>;
    return coeff *
...
static auto generate_nodes(num_collect::index_type num_interior_nodes,
    ...
    const auto interior_nodes =
        num_collect::rbf::generate_halton_nodes<double, 3>(num_interior_nodes);
