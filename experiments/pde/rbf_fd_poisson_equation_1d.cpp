...
auto main(int argc, const char** argv) -> int {
    ...
    const auto num_interior_nodes = parser.get<num_collect::index_type>(
        "rbf_fd_poisson_equation_1d.num_interior_nodes");
    const auto polynomial_order =
        parser.get<int>("rbf_fd_poisson_equation_1d.polynomial_order");
    const auto num_neighbors = parser.get<num_collect::index_type>(
        "rbf_fd_poisson_equation_1d.num_neighbors");
    const auto length_parameter_scale =
        parser.get<double>("rbf_fd_poisson_equation_1d.length_parameter_scale");
