...
#include "num_collect/functions/impl/expm1_maclaurin.h"
...
constexpr auto expm1_at_compile_time(T x) -> T {
    if (x < -static_cast<T>(1) || static_cast<T>(1) < x) {
        return exp(x) - static_cast<T>(1);
    }
    ...
    return -expm1_neg / (expm1_neg + static_cast<T>(1));
}
