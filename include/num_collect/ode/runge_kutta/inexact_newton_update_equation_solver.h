...
public:
    ...
    void update_jacobian(problem_type& problem, scalar_type time,
        ...
        inverted_value -= step_size * slope_coeff_ * problem.jacobian();
        coeff_inverse_ = static_cast<scalar_type>(1) / inverted_value;
        using std::isfinite;
    ...
    void iterate() {
        ...
        if constexpr (use_mass) {
            residual_ = problem_->mass() * (*solution_) -
                ...
                problem_->mass() * solution_offset_;
        } else {
            residual_ = (*solution_) -
                step_size_ * slope_coeff_ * problem_->diff_coeff() -
                solution_offset_;
...
public:
    ...
    void update_jacobian(problem_type& problem, scalar_type time,
        ...
        if constexpr (use_mass) {
            coeff_matrix_ =
                problem.mass() - step_size * slope_coeff_ * problem.jacobian();
        } else {
            coeff_matrix_ = jacobian_type::Identity(dim, dim) -
                step_size * slope_coeff_ * problem.jacobian();
        }
...
public:
    ...
    void update_jacobian(problem_type& problem, scalar_type time,
        ...
        if constexpr (use_mass) {
            ...
            coeff_matrix_.resize(dim, dim);
            coeff_matrix_.setIdentity();
        }
        coeff_matrix_ -= step_size_ * slope_coeff_ * problem_->jacobian();
...
public:
    ...
    void iterate() {
        ...
        const auto coeff_function = [this](const auto& target, auto& result) {
            ...
            result *= -step_size_ * slope_coeff_;
            if constexpr (use_mass) {
