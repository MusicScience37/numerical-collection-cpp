...
/*!
 * \file
 * \brief Definition of generate_rectangle_boundary_nodes_without_corners
 * function.
 */
#pragma once
...
/*!
 * \brief Generate nodes on the boundary of a rectangle without corners.
 *
...
[[nodiscard]] auto generate_rectangle_boundary_nodes_without_corners(
    ...
    const Scalar x_offset = dx / static_cast<Scalar>(2);
    const Scalar dy = (y_max - y_min) / static_cast<Scalar>(num_nodes_per_edge);
    const Scalar y_offset = dy / static_cast<Scalar>(2);
    ...
    if (order_in_clockwise) {
        for (index_type i = 0; i < num_nodes_per_edge; ++i) {
            nodes.push_back(Eigen::Vector2<Scalar>(
                x_min, y_min + y_offset + static_cast<Scalar>(i) * dy));
        }
        for (index_type i = 0; i < num_nodes_per_edge; ++i) {
            nodes.push_back(Eigen::Vector2<Scalar>(
                x_min + x_offset + static_cast<Scalar>(i) * dx, y_max));
        }
        for (index_type i = 0; i < num_nodes_per_edge; ++i) {
            nodes.push_back(Eigen::Vector2<Scalar>(
                x_max, y_max - y_offset - static_cast<Scalar>(i) * dy));
        }
        for (index_type i = 0; i < num_nodes_per_edge; ++i) {
            nodes.push_back(Eigen::Vector2<Scalar>(
                x_max - x_offset - static_cast<Scalar>(i) * dx, y_min));
        }
    } else {
        for (index_type i = 0; i < num_nodes_per_edge; ++i) {
            nodes.push_back(Eigen::Vector2<Scalar>(
                x_min + x_offset + static_cast<Scalar>(i) * dx, y_min));
        }
        for (index_type i = 0; i < num_nodes_per_edge; ++i) {
            nodes.push_back(Eigen::Vector2<Scalar>(
                x_max, y_min + y_offset + static_cast<Scalar>(i) * dy));
        }
        for (index_type i = 0; i < num_nodes_per_edge; ++i) {
            nodes.push_back(Eigen::Vector2<Scalar>(
                x_max - x_offset - static_cast<Scalar>(i) * dx, y_max));
        }
        for (index_type i = 0; i < num_nodes_per_edge; ++i) {
            nodes.push_back(Eigen::Vector2<Scalar>(
                x_min, y_max - y_offset - static_cast<Scalar>(i) * dy));
        }
    }
