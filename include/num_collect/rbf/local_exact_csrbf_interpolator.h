...
/*!
 * \file
 * \brief Definition of local_exact_csrbf_interpolator class.
 */
#pragma once
...
//! Tag of local_exact_csrbf_interpolator.
inline constexpr auto local_exact_csrbf_interpolator_tag =
    logging::log_tag_view("num_collect::rbf::local_exact_csrbf_interpolator");

/*!
 * \brief Class to interpolate using compactly supported RBF using localized
 * length parameter, without regularization.
 *
...
class local_exact_csrbf_interpolator : public logging::logging_mixin {
public:
    ...
    using kernel_matrix_type = Eigen::SparseMatrix<kernel_value_type>;
     ...
     * \param[in] rbf RBF.
     */
    explicit local_exact_csrbf_interpolator(
        distance_function_type distance_function = distance_function_type(),
        rbf_type rbf = rbf_type())
        : logging::logging_mixin(local_exact_csrbf_interpolator_tag),
    ...
    [[nodiscard]] auto interpolate(const variable_type& variable,
        ...
        for (std::size_t i = 0; i < variables_for_kernel.size(); ++i) {
            const kernel_value_type rbf_value =
            ...
            if (rbf_value != static_cast<kernel_value_type>(0)) {
                value += coeffs_(static_cast<index_type>(i)) * rbf_value;
            }
        }
        return value;
    }
...
private:
    ...
    Eigen::BiCGSTAB<kernel_matrix_type> kernel_matrix_solver_{};
