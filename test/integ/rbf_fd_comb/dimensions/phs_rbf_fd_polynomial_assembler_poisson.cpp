...
TEST_CASE("Poisson equation with phs_rbf_fd_polynomial_assembler in 1D") {
    ...
    const num_collect::index_type num_interior_nodes = 50;
    auto nodes = num_collect::rbf::generate_1d_halton_nodes<variable_type>(
        num_interior_nodes);
    nodes.push_back(0.0);
    nodes.push_back(1.0);
...
TEST_CASE("Poisson equation with phs_rbf_fd_polynomial_assembler in 2D") {
...
TEST_CASE("Poisson equation with phs_rbf_fd_polynomial_assembler in 3D") {
    ...
    auto test_function_laplacian = [](const variable_type& variable) -> double {
        constexpr double coeff = -3.0 * num_collect::base::pi<double> *
            num_collect::base::pi<double>;
        return coeff *
            (variable.array() * num_collect::base::pi<double>).sin().prod();
    };
    ...
    auto nodes =
        num_collect::rbf::generate_halton_nodes<typename variable_type::Scalar,
            variable_type::RowsAtCompileTime>(num_interior_nodes);
    const auto boundary_nodes =
        num_collect::util::generate_cuboid_boundary_nodes(
            variable_type(0.0, 0.0, 0.0), variable_type(1.0, 1.0, 1.0),
            num_boundary_nodes_per_edge);
    nodes.insert(nodes.end(), boundary_nodes.begin(), boundary_nodes.end());
    const auto interior_nodes =
        num_collect::util::vector_view<const variable_type>(nodes).first(
