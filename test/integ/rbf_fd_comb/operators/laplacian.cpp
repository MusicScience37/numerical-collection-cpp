...
/*!
 * \file
 * \brief Test of discretization of Laplacian operator.
 */
#include <Eigen/Core>
...
TEMPLATE_TEST_CASE_SIG(
    "Laplacian operator in 2D with different polynomial degrees", "",
    ...
    auto test_function_laplacian = [](const position_type& position) -> double {
    ...
    const solution_type expected_results = solution_type::NullaryExpr(
        test_nodes.size(), [&](num_collect::index_type i) {
            return test_function_laplacian(test_nodes[i]);
        });
