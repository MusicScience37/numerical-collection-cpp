...
/*!
 * \file
 * \brief Test of discretization of partial derivative operator.
 */
#include <Eigen/Core>
...
TEMPLATE_TEST_CASE_SIG(
    "Partial derivative operator in 2D with different polynomial degrees", "",
    ...
    using operator_type =
        num_collect::rbf::operators::partial_derivative_operator<position_type,
            0>;
    ...
    auto test_function_partial_derivative =
        [](const position_type& position) -> double {
        return num_collect::base::pi<double> *
            std::cos(position[0] * num_collect::base::pi<double>) *
            std::sin(position[1] * num_collect::base::pi<double>);
    };
    ...
    const solution_type expected_results = solution_type::NullaryExpr(
        test_nodes.size(), [&](num_collect::index_type i) {
            return test_function_partial_derivative(test_nodes[i]);
        });
