...
TEST_CASE(
    ...
    SECTION("solve a scalar problem with changing mass") {
        using problem_type =
    ...
    SECTION("solve a multi-variate problem without mass (dense)") {
        ...
        const Eigen::Vector2d actual =
            variable + step_size * solution.reshaped(2, stages) * update_coeffs;
    ...
    SECTION("solve a multi-variate without mass sparse") {
        ...
        slope_type solution =
            slope_type::Zero(variable.size() * formula_type::stages);
        solver.init(problem, time, step_size, variable, solution);
        REQUIRE_NOTHROW(solver.solve());
        ...
        const Eigen::Vector<double, stages> update_coeffs =
            formula_type::update_coeffs();
        const vector_type actual = variable +
            step_size * solution.reshaped(variable.size(), stages) *
                update_coeffs;
