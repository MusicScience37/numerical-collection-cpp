...
TEMPLATE_TEST_CASE(
    "num_collect::functions::impl::log_at_compile_time", "", float, double) {
    SECTION("x < 0") {
        constexpr auto x = static_cast<TestType>(-2);
        constexpr TestType value_at_compile_time =
            num_collect::functions::impl::log_at_compile_time(x);
    ...
    SECTION("x = 1e-10") {
        CHECK_CONSTEXPR_FUNCTION_RELATIVE((static_cast<TestType>(1e-10)),
            num_collect::functions::impl::log_at_compile_time, std::log);
    }
...
TEMPLATE_TEST_CASE("num_collect::functions::log", "", float, double) {
    SECTION("x < 0") {
        constexpr auto x = static_cast<TestType>(-2);
        constexpr TestType value_at_compile_time =
            num_collect::functions::log(x);
        const TestType value_runtime = num_collect::functions::log(x);
        CHECK(std::isnan(value_at_compile_time));
        CHECK(std::isnan(value_runtime));
    }
    ...
    SECTION("x = 1e-10") {
        CHECK_CONSTEXPR_FUNCTION_RELATIVE((static_cast<TestType>(1e-10)),
            num_collect::functions::log, std::log);
    }
    ...
    SECTION("x = 1") {
        CHECK_CONSTEXPR_FUNCTION_ABSOLUTE(
            (static_cast<TestType>(1)), num_collect::functions::log, std::log);
    }

    SECTION("x = 3") {
        CHECK_CONSTEXPR_FUNCTION_RELATIVE(
            (static_cast<TestType>(3)), num_collect::functions::log, std::log);
    }
