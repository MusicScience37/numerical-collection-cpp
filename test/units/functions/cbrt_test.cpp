...
TEMPLATE_TEST_CASE("num_collect::functions::cbrt", "", float, double) {
    SECTION("cbrt of negative number") {
        CHECK_CONSTEXPR_FUNCTION_RELATIVE((static_cast<TestType>(-1.234)),
            num_collect::functions::cbrt, std::cbrt);
    }
    ...
    SECTION("cbrt of infinity") {
        ...
        constexpr TestType value_at_compile_time =
            num_collect::functions::cbrt(x);
        const TestType value_runtime = num_collect::functions::cbrt(x);
        REQUIRE(std::isinf(value_at_compile_time));
        REQUIRE(value_at_compile_time > static_cast<TestType>(0));
        REQUIRE(std::isinf(value_runtime));
        REQUIRE(value_runtime > static_cast<TestType>(0));
    }
}

TEMPLATE_TEST_CASE(
    "num_collect::functions::cbrt (integers)", "", int, long long) {
    SECTION("cbrt of negative number") {
