...
/*!
 * \file
 * \brief Test of multi_quadric_rbf class.
 */
...
TEST_CASE("num_collect::rbf::rbfs::multi_quadric_rbf") {
    using num_collect::rbf::concepts::rbf;
    using num_collect::rbf::rbfs::multi_quadric_rbf;

    SECTION("check of concepts") {
        STATIC_REQUIRE(rbf<multi_quadric_rbf<double>>);
        STATIC_REQUIRE(rbf<multi_quadric_rbf<float>>);
    }

    SECTION("calculate a value") {
        constexpr double distance_rate = 1.2;
        constexpr double expected_value = 1.56204993518133;
        const multi_quadric_rbf<double> rbf;
