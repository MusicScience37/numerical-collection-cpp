...
/*!
 * \file
 * \brief Test of functional_preconditioned_gmres class.
 */
...
TEST_CASE("num_collect::linear::functional_preconditioned_gmres") {
    using num_collect::linear::functional_preconditioned_gmres;
    ...
    SECTION("solve") {
        ...
        functional_preconditioned_gmres<Eigen::Vector3d> solver;
    ...
    SECTION("set maximum number of iterations") {
        functional_preconditioned_gmres<Eigen::VectorXd> solver;

        CHECK_NOTHROW(solver.max_iterations(1));
        CHECK_THROWS(solver.max_iterations(0));
    }

    SECTION("set maximum number of dimensions of subspace") {
        functional_preconditioned_gmres<Eigen::VectorXd> solver;

        CHECK_NOTHROW(solver.max_subspace_dim(1));
        CHECK_THROWS(solver.max_subspace_dim(0));
    }
