...
TEST_CASE("num_collect::multi_double::quad") {
    ...
    SECTION("add a quad number with operator+ at compile time") {
        // 0x1.fb15fa86d92b228b6596fdb93fp+0
        constexpr auto a = quad(0x1.FB15FA86D92B2p+0, 0x0.28B6596FDB93Fp-52);
        // 0x1.22738c56c3ecf61e3f58931ec2p+0
        constexpr auto b = quad(0x1.22738C56C3ECFp+0, 0x0.61E3F58931EC2p-52);
    ...
    SECTION("add a quad number with operator+ at compile time (random)") {
        ...
        for (std::size_t i = 0; i < inputs.size(); ++i) {
            INFO("i = " << i);
            const auto& [a, b, sum_true] = inputs[i];
            const auto sum = outputs[i];
            CHECK_THAT(
    ...
    SECTION("subtract a quad number with operator- at compile time") {
        // 0x1.fb15fa86d92b228b6596fdb93fp+0
        constexpr auto a = quad(0x1.FB15FA86D92B2p+0, 0x0.28B6596FDB93Fp-52);
        // 0x1.22738c56c3ecf61e3f58931ec2p+0
        constexpr auto b = quad(0x1.22738C56C3ECFp+0, 0x0.61E3F58931EC2p-52);
    ...
    SECTION("subtract a quad number with operator- at compile time (random)") {
        ...
        for (std::size_t i = 0; i < inputs.size(); ++i) {
            INFO("i = " << i);
            const auto& [a, b, dif_true] = inputs[i];
            const auto dif = outputs[i];
            CHECK_THAT(
    ...
    SECTION("multiply quad numbers with operator* at compile time") {
        // 0x1.fb15fa86d92b228b6596fdb93fp+0
        constexpr auto a = quad(0x1.FB15FA86D92B2p+0, 0x0.28B6596FDB93Fp-52);
        // 0x1.22738c56c3ecf61e3f58931ec2p+0
        constexpr auto b = quad(0x1.22738C56C3ECFp+0, 0X0.61E3F58931EC2p-52);
    ...
    SECTION("multiply quad numbers with operator* at compile time (random)") {
        ...
        for (std::size_t i = 0; i < inputs.size(); ++i) {
            INFO("i = " << i);
            const auto& [a, b, prod_true] = inputs[i];
            const auto prod = outputs[i];
            CHECK_THAT(
                prod.high(), Catch::Matchers::WithinULP(prod_true.high(), 0));
            CHECK_THAT(prod.low(),
    ...
    SECTION(
        ...
        SECTION("quad * double") {
            ...
            for (std::size_t i = 0; i < inputs.size(); ++i) {
                ...
                const auto& [a, b, prod_true] = inputs[i];
                const auto prod = outputs[i];
                CHECK_THAT(prod.high(),
                    Catch::Matchers::WithinULP(prod_true.high(), 0));
                CHECK_THAT(prod.low(),
                    Catch::Matchers::WithinULP(prod_true.low(), ulp_limit));
            }
        }
    ...
    SECTION(
        "divide a quad number with another quad number with operator/ at "
        "compile time") {
        // 0x1.fb15fa86d92b228b6596fdb93fp+0
        constexpr auto a = quad(0x1.FB15FA86D92B2p+0, 0x0.28B6596FDB93Fp-52);
        // 0x1.22738c56c3ecf61e3f58931ec2p+0
    ...
    SECTION(
        ...
        for (std::size_t i = 0; i < inputs.size(); ++i) {
            INFO("i = " << i);
            const auto& [a, b, quotient_true] = inputs[i];
            const auto quotient = outputs[i];
            CHECK_THAT(quotient.high(),
                Catch::Matchers::WithinULP(quotient_true.high(), 0));
            CHECK_THAT(quotient.low(),
                Catch::Matchers::WithinULP(quotient_true.low(), ulp_limit));
    ...
    SECTION(
        "divide a quad number with a double number with operator/ at compile "
        "time"
        "(random)") {
        static constexpr auto inputs = std::to_array({
            // NOLINTBEGIN
    ...
    SECTION(
        "compare two quad numbers when higher digits differs at compile time") {
        constexpr auto a = quad(0x1.0000000000000p+0, 0x1.0p-53);
        constexpr auto b = quad(0x1.0000000000001p+0, 0x1.0p-53);

        STATIC_CHECK(a < b);
        STATIC_CHECK(a <= b);
        STATIC_CHECK_FALSE(a > b);
        STATIC_CHECK_FALSE(a >= b);
        STATIC_CHECK_FALSE(a == b);
        STATIC_CHECK(a != b);

        STATIC_CHECK_FALSE(b < a);
        STATIC_CHECK_FALSE(b <= a);
        STATIC_CHECK(b > a);
        STATIC_CHECK(b >= a);
        STATIC_CHECK_FALSE(b == a);
    ...
    SECTION(
        "compare two quad numbers with same higher digits and different lower "
        "digits at compile time") {
        constexpr auto a = quad(0x1.0p+0, 0x1.0000000000000p-53);
        constexpr auto b = quad(0x1.0p+0, 0x1.0000000000001p-53);
    ...
    SECTION("compare two quad numbers which are equal at compile time") {
        constexpr auto a = quad(0x1.0p+0, 0x1.0p-53);
        constexpr auto b = quad(0x1.0p+0, 0x1.0p-53);
    ...
    SECTION("compare a quad number with a double number at compile time") {
        constexpr auto a = quad(0x1.0000000000000p+0, 0x1.0p-53);
        constexpr auto b = 0x1.0000000000001p+0;
