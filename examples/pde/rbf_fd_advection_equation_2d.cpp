...
// Example: Solving 2D advection equation using RBF-FD method.

// TODO further description.
// TODO This example is unstable.
...
static auto test_function(const position_type& position, double time,
    ...
    const double relative_x =
        position.x() - advection_velocity * time - center_x;
...
static void solve_system(const ode_problem_type& problem,
    ...
    const std::string para_view_data_file_path =
        fmt::format("{}/rbf_fd_advection_equation_2d.pvd", output_directory);
    ...
    for (num_collect::index_type i = 0; i < num_interior_nodes; ++i) {
        whole_variable(i) = test_function(nodes[i], time, advection_velocity);
    }
    solution_type true_values = whole_variable;
    solution_type errors = solution_type::Zero(nodes.size());
    std::string vtp_file_name =
        fmt::format("rbf_fd_advection_equation_2d_{:04d}.vtp", time_index);
...
auto main(int argc, const char** argv) -> int {
    ...
    std::string_view config_file_path =
        "examples/pde/rbf_fd_advection_equation_2d.toml";
    if (argc == 2) {
    ...
    const auto num_interior_nodes = parser.get<num_collect::index_type>(
        "rbf_fd_advection_equation_2d.num_interior_nodes");
    const auto num_boundary_nodes_per_edge =
        parser.get<num_collect::index_type>(
            "rbf_fd_advection_equation_2d.num_boundary_nodes_per_edge");
    const auto polynomial_order =
        parser.get<int>("rbf_fd_advection_equation_2d.polynomial_order");
    const auto num_neighbors = parser.get<num_collect::index_type>(
        "rbf_fd_advection_equation_2d.num_neighbors");
    const auto advection_velocity =
        parser.get<double>("rbf_fd_advection_equation_2d.advection_velocity");
    const auto hyperviscosity_rate =
        parser.get<double>("rbf_fd_advection_equation_2d.hyperviscosity_rate");
    const auto time_step_size =
        parser.get<double>("rbf_fd_advection_equation_2d.time_step_size");
    const auto final_time =
        parser.get<double>("rbf_fd_advection_equation_2d.final_time");
