...
auto main(int argc, char** argv) -> int {
    ...
    solver_type solver;
    solver.compute(coeff, derivative_matrix, origin_vec);

    // Search for an optimal regularization parameter.
    const Eigen::VectorXd initial_solution =
        Eigen::VectorXd::Zero(config.upsampled_rows * config.upsampled_cols);
    ...
    visualize_result(origin, upsampled, "TV Regularization", "tv_admm");
